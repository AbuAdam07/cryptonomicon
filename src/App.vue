...
<script>
// [X]6 наличие состоянйи зависимых данных / 6
// [ ]2 при удалении остаётся подписка на загрузку тикера / 5
// [ ]4 запросы напрямую внутри компонента / 5
// [ ]5 обработка ошибок api / 5
// [X]8 при удалении тикера не удаляется localstorage / 4
// [ ]3 количество запросов / 4
// [X]1 одинаковый код в watch / 3
// [ ]9 localstorage и анонимные вкладки / 3
// [ ]7 график плохо выглядит, если будет много цен / 2
// [ ]10 магические строки и числа(URL, 5000 м/с задержкиб, ключ localStorage, количество на странице) / 1
...
export default {
  ...
  methods: {
    subscribeToUpdates(tickerName) {
      setInterval(async () => {
        const f = await fetch(
          `https://min-api.cryptocompare.com/data/price?fsym=${tickerName}&tsyms=USD&api_key=f10c564aa4bdaac890df8733024c11b98bc1dd78ad713f25478a75c929459455`
        );
        const data = await f.json();
        const tickerObj = this.tickers.find((t) => t.name === tickerName);
        if (tickerObj) {
